...
// Custom defines for configuration
...
#undef ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG
    ...
    public:
      Writer(Level level, const char *file, base::type::LineNumber line,
             ...
             base::type::VerboseLevel verboseLevel = 0) : m_msg(nullptr), m_level(level), m_file(file), m_line(line), m_func(func), m_verboseLevel(verboseLevel),
                                                          m_logger(nullptr), m_proceed(false), m_dispatchAction(dispatchAction)
      {
      }

      Writer(LogMessage *msg, base::DispatchAction dispatchAction = base::DispatchAction::NormalLog) : m_msg(msg), m_level(msg != nullptr ? msg->level() : Level::Unknown),
    ...
    namespace debug
    {
      ...
      public:
        ...
        public:
          ...
          StackTraceEntry(std::size_t index, const std::string &loc) : m_index(index),
                                                                       m_location(loc)
          {
          }
          std::size_t m_index;
